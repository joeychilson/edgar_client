...
#[module = "EDGAR.XBRL.Document"]
pub struct Document {
    facts: Vec<Fact>,
}

#[derive(Clone, NifStruct)]
#[module = "EDGAR.XBRL.Fact"]
pub struct Fact {
    context: Context,
    concept: String,
    value: Value,
...
fn parse_units(root: &roxmltree::Node) -> Result<HashMap<String, String>, String> {
    ...
    for unit_node in root.descendants().filter(|node| node.has_tag_name("unit")) {
            ...
            .find(|node| node.has_tag_name("divide"))
        {
            let numerator_measure = divide_node
                ...
                .and_then(|node| node.descendants().find(|n| n.has_tag_name("measure")))
